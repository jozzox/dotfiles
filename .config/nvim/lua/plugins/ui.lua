...
return {
  ...
  -- Theme
  {
    ...
    opts = {
      ...
      styles = {
        sidebars = "transparent", -- style for sidebars, see `:h highlight-groups`
        floats = "transparent", -- style for floating windows, see `:h highlight-groups`
      },
      on_highlights = function(_, colors)
        local theme = colors and colors.theme or {}
        -- Falls theme leer ist, gib einfach eine leere Tabelle zurück
        if not theme or vim.tbl_isempty(theme) then
          return {}
        end
        return {
